...
to_plot <- incidence_log_tidy %>%
  ...
  mutate(label = case_when(
    ! phase %in% c("no vaccine", "healthcare workers") ~ paste0(phase," (",supply*100,"%)"),
    TRUE ~ phase
  ))
order_labels <- c("no vaccine", "healthcare workers",
                  unique(to_plot$label[!to_plot$label %in% c("no vaccine", "healthcare workers")]))
...
#add no vax period and healthcare workers to all vaccination strategies
no_vax_cumulative <- unique(to_plot$cumulative_incidence[to_plot$time == min(to_plot$time[to_plot$phase == "healthcare workers"])-1])
healthcare_worker_cumulative <- max(to_plot$cumulative_incidence[to_plot$phase == "healthcare workers"])
to_plot <- to_plot %>%
  mutate(cumulative_incidence = case_when(
    phase == "no vaccine" ~ cumulative_incidence,
    phase == "healthcare workers" ~ cumulative_incidence + no_vax_cumulative,
    TRUE ~ cumulative_incidence + no_vax_cumulative + healthcare_worker_cumulative)
  )
